...
exports.resendOtp = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(200)
    .json({ status: "success", message: "A new verification code was sent" });
});
...
exports.refreshToken = asyncHandler(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new ApiError("No refresh token provided, please login again", 401),
    );
  }
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  await redis.set(`pwreset:${hashedResetToken}`, user.id, {
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({
    status: "success",
    message:
      "Password reset successful. Please login again on all your devices.",
  });
});
...
exports.getMe = asyncHandler(async (req, res) => {
  res
    .status(200)
    .json({ status: "success", data: { user: sanitizeUser(req.user) } });
});
